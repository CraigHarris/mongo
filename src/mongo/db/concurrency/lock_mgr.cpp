...
namespace mongo {
    ...
    void LockManager::setPolicy(Transaction* tx, const Policy& policy, Notifier* notifier) {
        ...
        if (kPolicyReadersOnly == policy || kPolicyWritersOnly == policy) {
            ...
            if (0 < (this->*numBlockers)()) {
                if (notifier) {
                    (*notifier)(_systemTransaction);
                }
                do {
                    _policyLock.wait(lk);
                } while (0 < (this->*numBlockers)());
            }
        }
    }
...
//     unique_lock<boost::mutex> lk(_mutex);
    ...
    void LockManager::_acquireInternal(LockRequest* lr,
        ...
        do {
            ...
            resourceStatus = _conflictExists(lr->requestor, lr->mode, lr->resId, lr->slice, queue, conflictPosition);
    ...
    LockManager::LockStatus LockManager::_findLock(const Transaction* holder,
        ...
        map<ResourceId,LockRequest*>::const_iterator resourceLocks = _resourceLocks[slice].find(resId);
        if (resourceLocks == _resourceLocks[slice].end()) { return kLockResourceNotFound; }
